...
async function loadPostPage(lang, slug) {
  ...
  try {
    // 1. Fetch metadata and the post template
    ...
    // 4. Trigger the Sidebar for this specific language
    // This fills the <aside id="latest-sidebar"> we just added
    await renderLatestSidebar(lang);

    // 5. Inject Post Metadata
    ...
    // 6. Fetch and inject the actual post content
    const contentResponse = await fetch(`./posts/${lang}/${slug}.html`);
